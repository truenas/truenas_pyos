...
the next id. Errors that concern the mount being enumerated rather than one of
its children are raised as `OSError` — in particular, a scoped iteration
(`mnt_id=` other than `LSMT_ROOT`) whose own mount is unmounted mid-iteration
fails with `ENOENT` from `listmount(2)`.
...
Batch per-credential access probes against a list of path components.  Each
credential is tested against each component, and only the denials are
returned.  Access checks dispatch through the filesystem's own permission
inode operation, so filesystem-native ACLs (e.g. ZFS NFSv4) are honored, not
just mode bits.

The default `mode` (`X_OK`) performs execute-traversal probes — pass a list
of ancestor directories to pre-flight whether a principal could reach a
leaf.  Pass `mode=R_OK` or `mode=W_OK` (or any OR of `R_OK | W_OK | X_OK`)
together with a single-element `components` list to probe read or write
access on a leaf instead.
...
# Execute-traversal pre-flight: can each cred walk to /mnt/tank/share/sub/file?
ancestors = [b"/mnt", b"/mnt/tank", b"/mnt/tank/share", b"/mnt/tank/share/sub"]
failures = truenas_os.check_path_access(creds=creds, components=ancestors)
for f in failures:
    print(f"{f.id_name} denied at {f.failing_component!r}: errno={f.errnum}")

# Single-path read probe: can libvirt read this ISO?
libvirt = truenas_os.create_cred_entry("libvirt-qemu", 64055, 64055, ())
failures = truenas_os.check_path_access(
    creds=[libvirt], components=[b"/mnt/tank/iso/debian.iso"],
    mode=os.R_OK, path_must_exist=True,
)
```
...
- `mode` (int, keyword-only, optional, default=`X_OK`): Bitmask forwarded to
  `faccessat2(2)`.  Must be a non-zero subset of `R_OK | W_OK | X_OK`.
  `F_OK` (existence-only) is rejected — use `path_must_exist=True` instead.
...
- `ValueError` if `creds` is empty, or `mode` is 0 or contains bits outside
  `R_OK | W_OK | X_OK`.
