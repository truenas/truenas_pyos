...
These are the tests that matter most. Every buffer handed to a preadv2/pwritev2
is kernel-visible from submission until its completion reaps, so the op slot
owns it, not the caller. A prepared-but-dropped handle must give its slot and pin
...
def test_buffer_cannot_be_resized_while_in_flight(tmp_path):
    ...
    with ring_env(tmp_path) as (r, dirfd):
        ...
        buf = bytearray(64)
        r.submit([r.prep_preadv2(fh, [buf], 0)])         # not reaped yet
...
def test_buffer_resizable_again_after_completion(tmp_path):
    ...
    with ring_env(tmp_path) as (r, dirfd):
        fh = drive_one(r, r.prep_openat2(dirfd, b'f', os.O_RDONLY))
        buf = bytearray(8)
        drive_one(r, r.prep_preadv2(fh, [buf], 0))
...
def test_pin_released_after_cancel(tmp_path):
    ...
    with ring_env(tmp_path) as (r, dirfd):
        fh = drive_one(r, r.prep_openat2(dirfd, b'f', os.O_RDONLY))
        buf = bytearray(4096)
        (ud,) = r.submit([r.prep_preadv2(fh, [buf], 0)])
        with pytest.raises(BufferError):
            buf.extend(b'z')
        r.cancel(ud)
        import select
...
@requires_io_uring
def test_path_object_kept_alive_by_the_op(tmp_path):
    """The op holds its own reference to the converted path bytes (the SQE
    points into them), so the caller may drop its path object between prep and
    submit and the operation still resolves the right file."""
    (tmp_path / 'kept').write_bytes(b'alive')
    with ring_env(tmp_path) as (r, dirfd):
        name = b''.join([b'ke', b'pt'])     # a fresh, uncached bytes object
        h = r.prep_openat2(dirfd, name)
        del name
        gc.collect()
        fh = drive_one(r, h)                # submits after the caller's drop
        buf = bytearray(5)
        assert drive_one(r, r.prep_preadv2(fh, [buf])) == 5
        assert bytes(buf) == b'alive'
        drive_one(r, r.prep_close(fh))


@requires_io_uring
def test_path_ref_released_on_every_path(tmp_path):
    """The owned path-bytes ref is dropped when the completion reaps and when
    a prepped handle is dropped -- never leaked."""
    import sys
    (tmp_path / 'file').write_bytes(b'x')
    with ring_env(tmp_path) as (r, dirfd):
        pb = b''.join([b'fi', b'le'])       # fresh bytes: FSConverter reuses it
        gc.collect()
        base = sys.getrefcount(pb)
        for _ in range(50):
            drive_one(r, r.prep_statx(dirfd, pb))   # released at reap
            dropped = r.prep_openat2(dirfd, pb)
            del dropped                              # released on drop
        gc.collect()
        assert sys.getrefcount(pb) == base
...
def test_submit_refuses_a_submitted_handle(tmp_path):
    ...
    with ring_env(tmp_path) as (r, dirfd):
        fh = drive_one(r, r.prep_openat2(dirfd, b'f', os.O_RDONLY))
        handle = r.prep_preadv2(fh, [bytearray(8)], 0)
...
def test_submitted_handle_inert_across_slot_reuse(tmp_path):
    ...
    live op's pinned buffer from a destructor.
    """
    (tmp_path / 'f').write_bytes(b'x' * 8)
    with ring_env(tmp_path, entries=1) as (r, dirfd):
        fh = drive_one(r, r.prep_openat2(dirfd, b'f', os.O_RDONLY))

        first = r.prep_preadv2(fh, [bytearray(8)], 0)
        drive(r, [first])               # submit + reap; slot is now free again
        assert r.inflight == 0

        bufB = bytearray(8)
        second = r.prep_preadv2(fh, [bufB], 0)   # reuses the one slot
        with pytest.raises(BufferError):
    ...
    with ring_env(tmp_path) as (r, dirfd):
        ...
        with ring_env(tmp_path) as (other, other_dirfd):
            of = drive_one(other, other.prep_openat2(other_dirfd, b'f',
                                                     os.O_RDONLY))
            foreign = other.prep_preadv2(of, [bytearray(8)], 0)
...
def test_submit_refuses_a_non_handle(tmp_path):
    ...
    with ring_env(tmp_path) as (r, dirfd):
        fh = drive_one(r, r.prep_openat2(dirfd, b'f', os.O_RDONLY))
        good = r.prep_preadv2(fh, [bytearray(8)], 0)
...
def test_transactional_unwind_consumes_nothing(tmp_path):
    ...
    with ring_env(tmp_path, entries=8) as (r, dirfd):
        fh = drive_one(r, r.prep_openat2(dirfd, b'f', os.O_RDONLY))
        for _ in range(200):
            good = r.prep_preadv2(fh, [bytearray(4)], 0)
            with pytest.raises(TypeError):
...
def test_batch_larger_than_ring_is_rejected(tmp_path):
    ...
    with ring_env(tmp_path, entries=4) as (r, dirfd):
        ...
        handles = [r.prep_preadv2(fh, [bytearray(16)], 0) for _ in range(4)]
        with pytest.raises(BlockingIOError):
            r.prep_preadv2(fh, [bytearray(16)], 0)   # pool exhausted
        assert r.inflight == 0
...
def test_op_slots_recycle_without_confusion(tmp_path):
    ...
    with ring_env(tmp_path, entries=8) as (r, dirfd):
        for i in range(300):
            ...
            body = b'payload%d' % i
            assert drive_one(r, r.prep_pwritev2(fh, [body], 0)) == len(body)
            buf = bytearray(64)
...
def test_close_with_operations_in_flight(tmp_path):
    ...
    with ring_env(tmp_path) as (r, dirfd):
        ...
        r.submit([r.prep_preadv2(fh, [buf], 0) for buf in bufs])   # not reaped
...
def test_close_with_prepared_handles_outstanding(tmp_path):
    ...
    (tmp_path / 'f').write_bytes(b'x' * 4096)
    with ring_env(tmp_path, entries=16) as (r, dirfd):
        fh = drive_one(r, r.prep_openat2(dirfd, b'f', os.O_RDONLY))
        bufs = [bytearray(4096) for _ in range(3)]
        prepared = [r.prep_preadv2(fh, [buf], 0) for buf in bufs]   # prepared, not submitted
        with pytest.raises(BufferError):
